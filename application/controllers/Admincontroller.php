...
class Admincontroller extends CI_Controller {
	...
	public function dashboard()
	{
		...
		$data['orders']=$this->action->select_all("orders", "created_at DESC");
	...
	public function stripeSettings(){
		$data['users']=$this->action->select_all("users","id asc");
		$wherech['archive'] = 0;
 		$data['channels']=$this->Crud_model->get_data("*","channels",$wherech,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "id" ,$odr_method="DESC");
		$where['role'] = 'customer';
		$data['customer'] = $this->Crud_model->get_data("*","users",$where);
		$data['stripeData'] = $this->Crud_model->get_data("","stripesettings","",True);
		$this->load->view('admin/stripeSettings', $data);
	}

	public function saveSettings(){
		$formData = $this->input->post();
		if(empty($formData["stripeId"])){
			$this->Crud_model->add_data(["publicKey"=>$formData["publicKey"],"privateKey"=>$formData['privateKey']],"stripesettings");
			$this->session->set_flashdata("success","Stripe Settings has been saved");
			redirect("Admincontroller/stripeSettings");
		}else{
			$this->Crud_model->update_data(["stripeId"=>$formData["stripeId"]],["publicKey"=>$formData["publicKey"],"privateKey"=>$formData['privateKey']],"stripesettings");
			$this->session->set_flashdata("success","Stripe Settings has been saved");
			redirect("Admincontroller/stripeSettings");
		}
	}
	...
	public function commissionSettings(){
		...
		$where['role'] = 'customer';
		$data['customer'] = $this->Crud_model->get_data("*","users",$where);
		$data['commission'] = $this->Crud_model->get_data("","commission","",True);
		$this->load->view('admin/commission', $data);
	}

	public function saveCommission(){
		$formData = $this->input->post();
		if(empty($formData["commissionId"])){
			$this->Crud_model->add_data(["commission"=>$formData["commission"]],"commission");
			$this->session->set_flashdata("success","Commission has been saved");
			redirect("Admincontroller/commissionSettings");
		}else{
			$this->Crud_model->update_data(["commissionId"=>$formData["commissionId"]],["commission"=>$formData["commission"]],"commission");
			$this->session->set_flashdata("success","Commission has been saved");
			redirect("Admincontroller/commissionSettings");
		}
	}
	...
	public function order($action="", $id="")
	{
		if($action=="view"){
			...
			$data['single_order']=$this->action->select("orders", array("id"=>$id));
			$this->load->view('admin/item/editOrder', $data);	
		}
		else if($action=="delete"){
	...
	public function saveEditOrder(){
		...
		if(empty($formData["statuschange"])){
			$this->session->set_flashdata("error","Please select status.");	
			$this->load->library('user_agent');
		    redirect($this->agent->referrer());
		}else{
			$data = array(
				"payment_status" => $formData["statuschange"],
				"trackingInformation" => $formData["trackingInformation"]
			);
			$this->Crud_model->update_data(["order_id"=>$formData["orderId"]],$data,"orders");
			redirect("Admincontroller/order");
		}
	}
	...
	{			
			...
			$data['concierges'] = $this->action->select_all("concierge","id desc");
			$this->session->set_flashdata('successemail', "Email successfully sent.");
	 	    $this->load->view('admin/concierge', $data);
	}
	...
	{	
		...
		if(!empty($_GET['customerId'])){
			 $query1 = "SELECT * FROM chatstart WHERE archive = '".$archiveValue."' AND fromId='".$_GET['customerId']."' OR (toId='".$_GET['customerId']."' AND archive = '".$archiveValue."')";
		     $data['allConversation'] = $this->db->query($query1)->result();
		}else{
			$data['allConversation'] = $this->Crud_model->get_data("","chatstart",["archive"=>$archiveValue]);
		}
		// echo "<pre>";print_r($data['allConversation']);exit;
		$data['users']=$this->action->select_all("users","id asc");
		$wherech['archive'] = 0;
 		$data['channels']=$this->Crud_model->get_data("*","channels",$wherech,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "id" ,$odr_method="DESC");
		$where['role'] = 'customer';
	...
	public function deleteConversation($conversationId){
		if(!empty($conversationId)){
			$this->Crud_model->delete_data("chat",['chatStartId'=>$conversationId]);
			$this->Crud_model->delete_data("chatstart",['chatStartId'=>$conversationId]);
			redirect("Admincontroller/chat_record");
		}else{
			redirect(base_url());	
		}
	}

	public function chatRecordDetail($conversationId){
		$data['users']=$this->action->select_all("users","id asc");
		$wherech['archive'] = 0;
 		$data['channels']=$this->Crud_model->get_data("*","channels",$wherech,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "id" ,$odr_method="DESC");
		$where['role'] = 'customer';
		$data['customer'] = $this->Crud_model->get_data("*","users",$where);
		$data['chatData'] = $this->Crud_model->get_data("","chat",["chatStartId"=>$conversationId]);
		$archiveValue = 0;
		if(!empty($_GET['archive'])){
			$archiveValue = 1;
		}
	...
	public function addArchive($chatStartId){
		if(!empty($chatStartId)){
			$this->Crud_model->update_data(['chatStartId'=>$chatStartId],['archive'=>1],"chatstart");
		}
		redirect($this->agent->referrer());
	}

	public function unArchive($chatStartId){
		if(!empty($chatStartId)){
			$this->Crud_model->update_data(['chatStartId'=>$chatStartId],['archive'=>0],"chatstart");
		}
		redirect($this->agent->referrer());
	}

	public function chat_record_search()
	{
		$data['users'] = $this->action->select_all("users","id asc");

		// $wherechnnels['sender_id'] = $_POST['customer_id'];
		// $wherechnnels['receiver_id'] = $_POST['customer_id'];
		$wherechnnels = "sender_id = ".$_POST['customer_id']." or "."receiver_id =".$_POST['customer_id']." AND archive = 0";
		$data['channels']=$this->Crud_model->get_data("*","channels",$wherechnnels,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "id" ,$odr_method="DESC");
	...
	public function ajax_call()
	{
		...
		if($action=="remove_channel_data"){
			$where['id']=$this->input->post("channel_id");
			$this->Crud_model->delete_data('channels',$where);
			echo 'success';
		}
		...
		if($action=="archive"){
			$where['id']=$this->input->post("channel_id");
			$data['archive'] = 1;
			$this->Crud_model->update_data($where,$data,"channels");
			echo 'success';
		}
		...
		if($action=="block_channel"){
			...
			$data['chat_status'] = 0;
			$this->Crud_model->update_data($where,$data,"users");
			echo 'success';
		}
	...
	public function transcation(){
		...
		$this->load->view('admin/transcation',$data);
	}

	public function search_orders(){
		
		$whereorder['u_id'] = $_POST['customer_id'];
		if(!empty($_POST['date_to'])){
			$whereorder['created_at'.'>='] = $_POST['date_to'];	
		}

		if(!empty($_POST['date_from'])){
			$whereorder['created_at'.'<='] = $_POST['date_from'];
		}
		$data['orders'] = $this->Crud_model->get_data("*","orders",$whereorder,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "created_at" ,$odr_method="DESC");
		// echo $this->db->last_query();
		// die;
		$whereitems['item_user_id'] = $_POST['customer_id'];
		$data['products'] = $this->Crud_model->get_data("*","items",$whereitems,$single_row = false,$join = array(), $like = array(),$or_where = "",$where_in = "", $order_by = "created_at" ,$odr_method="DESC");
	...
	public function visitor(){
		...
		$analyticsreport = $this->initializeAnalytics_report();
		$totalcounty = $this->getReport($analyticsreport);

		$data['users'] =  $visitor[0][0];
		$data['sessions'] =  $sessions[0][0];
		$data['pagesview'] =  $pagesview[0][0];
		$data['newusers'] =  $newusers[0][0];
		$data['country'] = $totalcounty['reports'][0]['data']['rows'];
	...
	public function initializeAnalytics_report()
	{
	...
	public function getReport($analytics) {
	  ...
	  $dateRange->setStartDate("200daysAgo");
	  $dateRange->setEndDate("today");

	  // Create the Metrics object.
	  $sessions = new Google_Service_AnalyticsReporting_Metric();
	  $sessions->setExpression("ga:Visitors");
	  $sessions->setAlias("Visitors");
