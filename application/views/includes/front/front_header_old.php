						...
						if(!empty($this->session->userdata('auth_user')[0]->id)){
							...
							$allConversation = $this->db->query($query)->result();
							 ...
							 <?php if(!empty($allConversation[0])){ if(!empty($allConversation[0]->productSlug)){
								$url = base_url('Chat/index/').$allConversation[0]->productSlug.'/'.$allConversation[0]->toId;
							}else{ 
								$url = base_url('Chat/index/profile').'/'.$allConversation[0]->toId;
							}
							}else{
								$url = "#";	
							}
...
?>
						<div class="mouse">
							<div id="mynotify" class="Notify">
								<h3>Messages</h3>
								<a class="viewall" href="<?php echo $url; ?>"  >View All</a>&nbsp;<a href="javascript:void(0)" class="closebtn" onclick="closeNotification()">&times;</a>


<?php if(!empty($fromId)){ foreach ($allConversation as $convo) { ?>
<div class="first">
													<img src="<?php echo base_url(); ?>themes/front/images/download.png" alt=""/>
													<div class="text-area">
														<?php if(!empty($convo->productSlug)){
															$url = base_url('Chat/index/').$convo->productSlug.'/'.$convo->toId;
														}else{ 
															$url = base_url('Chat/index/profile').'/'.$convo->toId;
														}
															?>
														<a href="<?php echo $url; ?>">
															<?php 
                                $userNames = getUserName($convo->toId);
                                echo $userNames->first_name ?> has sent you message.
															<p><?php $lastMessage = getLastMessage($convo->chatStartId); echo $lastMessage->message ?></p>
														</a>
											...
											<ul class="nav navbar-nav">
												...
												<?php if(null !== $this->session->userdata('auth_user')){ ?>
													<li class="">
														...
														<a href="<?php echo base_url(); ?>chat_mobile" onclick="return openNotification()">
												...
												<?php if(null !== $this->session->userdata('auth_user')){ ?>
													<li class="">
														<a href="<?php echo base_url(); ?>notification" onclick="return openNotification_alert()">
															...
															<span id='notificationremove' class="<?php echo $notify_alert; ?>">
																Notifications
															</span>
